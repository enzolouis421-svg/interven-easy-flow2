  ...
  const saveSignature = async () => {
    ...
    if (!user) {
      toast({
        variant: "destructive",
        title: "Erreur",
        description: "Vous devez être connecté pour enregistrer une signature",
      });
      return null;
    }
        ...
        canvas.toBlob(
          (blob) => {
            if (blob) {
              ...
              reject(new Error("Impossible de convertir la signature en image"));
            }
          },
          "image/png",
          1.0
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      if (data.html) {
        ...
        const filename = `intervention-${formData.titre?.replace(/[^a-zA-Z0-9]/g, '-') || id}-${new Date().toISOString().split('T')[0]}.pdf`;
